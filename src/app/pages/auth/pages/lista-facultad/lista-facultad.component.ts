...
import { GetFacultadComponent } from '../get-facultad/get-facultad.component';
...
export class ListaFacultadComponent   implements AfterViewInit, OnInit {
  ...
  constructor(
    ...
    private _serviceFacultad: FacultadService,
  ...
  cargarFacultad() {
    if(this.isSuperAdmin){
      this._serviceFacultad.get().subscribe({
        next: (data) => {
      ...
      this._serviceUserUni.getByUsuario(this.idUser).subscribe({
        next: (data) => {
          this._serviceFacultad.getByUniversidad(data.at(0).idUniversidad).subscribe({
            next: (data2) => {
  ...
  GetFacultad(){
    this._serviceFacultad.get().subscribe({
      next: (data) => {
        const maximo = data.length > 0 ? ( Number(data.reduce((max, item) => item.codigoFacultad > max ? item.codigoFacultad : max, data[0].codigoFacultad)) +1):1;
        const consecutivo = maximo.toString().padStart(4, '0');
        const fechaFormateada = this.datePipe.transform(new Date(), 'yyyy-MM-dd');
        const sede: Facultad = {
          id:null,
          nombreSede:'',
          codigoFacultad:consecutivo,
          nombre:'',
  ...
  GetFacultadEdit(datos: Facultad){
    this.dialog.open(GetFacultadComponent,{
      disableClose: true,
      width:"600px",
      data: datos
    }).afterClosed().subscribe(resultado => {
  ...
  DeleteFacultad(datos: Facultad){
    this.dialog
      .open(GetEliminarComponent, {
        disableClose: true,
        data: {
          componeteEliminar: 'eliminar facultad',
          tituloAccion: 'Eliminar Facultad',
          datos: datos,
        },
      })
      .afterClosed()
      .subscribe((resultado) => {
        if (resultado === 'Eliminar') {
          this._serviceFacultad.delete(datos.id).subscribe({
            next: (data) => {

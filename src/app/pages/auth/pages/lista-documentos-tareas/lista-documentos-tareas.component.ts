...
import { SolicitudTareaDocumentoService } from '@app/services/solicitudes/solicitud-tarea-documento.service';
...
@Component({
  selector: 'app-lista-documentos-tareas',
  templateUrl: './lista-documentos-tareas.component.html',
  styleUrls: ['./lista-documentos-tareas.component.scss'],
  providers : [
...
export class ListaDocumentosTareasComponent  implements AfterViewInit, OnInit {
  showUpload = false;
  ...
  dataSource = new MatTableDataSource<SolicitudTareaDocumento>();
  ...
  constructor(
    ...
    private _serviceDocumento: SolicitudTareaDocumentoService,
  ...
  cargarArchivos() {
    this._serviceDocumento.getByIdSolicitud(this.idSolicitud).subscribe({
        next: (data) => {
  ...
  detectarTipoYNombreArchivo(url: string): void {
    ...
    const modelo: SolicitudTareaDocumento= {
      id: 0,
      tipoArchivo: tipoArchivo,
      nombreArchivo:nombreArchivoSinPath,
      archivo: url,
      idSolicitudTarea: this.idSolicitud,
  ...
  EimnarArchivo(datos: SolicitudTareaDocumento){
    this.dialog
